/* run.config
   LOG: @PTEST_NAME@.out.c
   LOG: saida_harness_@PTEST_NAME@.c
   OPT: -lib-entry -saida -saida-tricera-opts="-acsl -solutionReconstruction:wp" -saida-keep-tmp -saida-out=@PTEST_NAME@.out.c
*/
/*
  Tests harness generation for nested structs in requires/ensures
  clauses is handled correctly. TriCera's default reconstruction emits an
  invalid aggregate expression for this case, while its supported WP
  reconstruction produces valid ACSL.
 */

struct Si {
  int x;
};

struct So {
  struct Si inner;
};

struct So s;

int select_inner_x(struct So structs) {
  return structs.inner.x ;
}

/*@
  requires s.inner.x >=0;
  ensures \result == s.inner.x ;
*/ 
int main(void) {
  return select_inner_x(s);
}
